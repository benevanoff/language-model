...
std::vector<std::string> parse_utterances_from_dialog_csv() {
    ...
    while (ifs.good()) {    
        try {
            // read each line from the data file
            std::getline(ifs, line);
            auto parsed = parseUtterances(line);
            utterances.push_back(parsed.first + " <stop> " + parsed.second + " <stop>");
            // std::cout << "! " << utterances.at(utterances.size()-1) << " !" << std::endl;
        } catch (std::exception e) {}
    }
        ...
        for (int64_t i = 0; i < list_array->length(); ++i) {
            ...
            for (int64_t j = start; j < end; ++j) {
                if (string_array->IsNull(j)) {
                    ...
                    dialog += string_array->GetString(j) + " <stop>";
                    if (j != end-1) dialog += " ";
                }
            }
            utterances.push_back(dialog);
